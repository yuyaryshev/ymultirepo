...
export function npmConfigGet(name) {
    return execSync(`npm config get ${name}`,{encoding:"utf-8"}).trim();
}

export function getLocalPackagesPathsObj() {
    if(!localPackagesPaths) {
        const paths = npmConfigGet('local_packages_folders')
            .split(';')
            .map(s=>s.trim())
            .filter(s => s.length);
...
export function printSnippet() {
    const prefix = npmConfigGet('prefix').split("\\").join("/");
    if(prefix) {
        const snippet = `
// Put the following snippet into pnpmfile.js:
// import {ymultirepoRemap, ymultirepoScan}  from "file://${prefix}/node_modules/ymultirepo/lib/index.mjs";
// const {ymultirepoRemap, ymultirepoScan}  = require("${prefix}/node_modules/ymultirepo/lib/index.cjs");
...
`;
        ...
        console.log(`ERROR Could't generate snippet for pnpmfile.js because 'npm config prefix' is missing.`);
    }
}